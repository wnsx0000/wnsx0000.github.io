...
- scale 내부의 값은 NPU에서 INT8 MatMul을 한다. 즉, activation을 scaling factor $s$로 quantize하고 W8A8 MatMul 연산을 한다.

- scale 외부의 값인 outlier들을 따로 추출해 CPU에서 FP MatMul을 한다. 즉, runtime에 outlier들을 추출해 dense한 tensor를 구성하고, CPU에서 연산한 후 NPU 연산 결과에 병합한다. 아래 그림과 같이 outlier들과, 대응되는 weight를 추출해 연산한다.

![](/assets/img/posts/2025-08-09-llm-npu/shadow.png){: width="450" }

이를 수식으로 나타내면 아래와 같다. $x$는 원본 float activation, $w$는 INT8 weight, $s$는 scaling factor, $\odot$는 MatMul, extract()는 activation outlier를 추출하는 함수이다. scaling factor $s$는 scale에 대한 threshold로 기능하므로 중요한데, 이는 offline에서 대규모 corpora에 대해 profile하여 구했다고 한다.

<!-- s를 구체적으로 어떻게 구한 것인지, 어떤 데이터셋을 활용했는지 밝히지 않았다. -->
...
이때 실험 결과 outlier는 전체 channel(feature) 중 0.1% ~ 0.3% 수준으로, 아주 sparse하다고 한다. 이에 따라 dense한 tensor로 구성해 CPU에서 연산하는 것이 NPU에서 연산하는 것보다 빠르고, NPU에서 연산이 수행되므로 완전히 숨겨진다고(totally hidden) 한다.
...
근데 quantization은 inference engine에서 적용하는 건가..? 미리 적용해서 배포하는 거 아닌가? 아님 둘 다 가능한가? 근데 배포한 quantization과 inference engine의 quantization 기법이 다르면 안되는 거 아닌가. 그냥 배포한 quantization이 per-tensor이면 다 가능하다는 말인 듯. 물론 scaling factor는 quantization

일단 NPU 활용에 따라 per-tensor quantization을 적용해야 한다.
