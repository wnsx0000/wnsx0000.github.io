...
NPU 활용에는 group quantization이 적절하지 않으므로, 논문에서는 accuracy 저하가 없는 per-tensor quantization 기법인 Shadow Outlier Execution을 제안한다. shadow outlier execution의 동작은 아래와 같다.
...
특정 chunk의 layer별로 CPU, NPU 각각의 실행 시간 차이가 있을 수 있으므로.. naive하게 실행할 경우 bubble이 발생한다.



<!-- 여기에서도 CPU/GPU라고 하는데.. 그림엔 또 CPU만 쓰여 있다. -->
