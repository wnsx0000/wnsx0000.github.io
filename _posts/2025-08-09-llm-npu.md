...
- Activation outlier의 존재에 따른 quantization 적용 문제

    LLM activation에는 outlier들이 존재할 수 있으므로 per-tensor 등 granularity가 높은 quantization 기법을 적용하는 경우 accuracy가 떨어지는 문제가 발생할 수 있다. 이에 따라 group quantization 또는 outlier를 고려하는 per-tensor quantization 기법인 SmoothQuant 등을 적용할 수 있다.

    하지만 아래 그래프(Figure 4)에서 확인할 수 있듯이, NPU를 활용하는 경우 accuracy 유지를 위해 group quantization을 적용하는 K-Quant, AWQ 등에서는 latency overhead가 크다. 이는 group quantization의 경우 각 group 별 scaling을 적용하므로 group 별 MatMul을 수행해야 하고, 연산 이후 각 group에 대한 결과를 FP operation을 사용해 aggregate해야 하기 때문이다(Figure 3). 이 경우 NPU의 computation capability를 충분히 활용하지 못한다.

    또한 단순 per-tensor quantization을 적용하는 경우 심각한 accuracy 저하가 발생하고, SmoothQuant에서도 3.9% ~ 8.4%의 accuracy 저하가 발생한다.

![](/assets/img/posts/2025-08-09-llm-npu/NPU vs LLM 2.png){: width="450" }

<!-- 애초에 모델 성능이 낮은 듯하다. HellaSwag여서 그런 거 같기도. HelloSwag는 오타인가? -->
<!-- SmoothQuant에서는 accuracy 저하가 왜 발생하나? -->

- On-device LLM inference에서의 FP operation 활용

    아래 표와 같이 quantization을 적용한 전형적인 LLM workflow에서는 accuracy 유지를 위해 linear layer에 대해서는 INT MatMul을 수행하고, attention과 layernorm 등 기타 부분들에서는 FP operation을 수행한다. 즉, INT 연산이 효율적인 NPU에서는 LLM의 연산들이 효율적으로 수행되기 어렵다.
...
NPU 활용에는 group quantization이 적절하지 않으므로, llm.npu에서는 accuracy 저하가 없는 per-tensor quantization 기법인 **Shadow Outlier Execution**을 적용한다. shadow outlier execution의 동작은 아래와 같다.

<!-- 이 quantization은 linear layer에만 적용하는 것으로 보인다. -->
<!-- 여기에서 per-tensor는 결국 chunk별로 적용되는 것인가? -->
...
<!-- channel == feature? -->
<!-- NPU 연산이 대부분이고, CPU 연산은 극히 일부이므로 hidden되는 것을 이해할 수 있다. -->
...
1. NPU/CPU의 분리된 Memory Space에 따른 Memory Footprint 증가
    ...
    mobile SoC는 heterogeneous processor들에 대해 unified physical memory를 활용하지만, 각 memory space는 분리되어 있다. 이에 따라 CPU 연산을 위해서는 CPU memory space에 weight copy가 추가로 존재해야 하는데, 이 경우 memory footprint가 2배 가까이 증가하게 된다.
    ...
    <!-- memory footprint만이 아니라, latency가 34.3% 가량 줄었다고 하는데... memory에 load하는 시간이 줄었기 때문인가? -->
...
2. Synchronization Overhead
    ...
    하지만 관찰에 의하면 대부분의 outlier들은 accuracy에 대해 중요도(CPU 연산이 필요한 정도)가 높지 않다. 왼쪽 아래 그래프는 각 layer에 대한 중요도를 나타낸 것으로, 이때의 중요도는 가장 큰 outlier 값을 대응되는 scaling factor로 나눈 값이다. 즉, 중요도가 높을수록 단순 per-tensor quantization 시에 quantization error가 발생하기 쉽고, 이에 따라 CPU에서 연산되어야 함을 나타낸다.

    이에 따라 llm.npu에서는 scaling factor를 구할 때와 마찬가지로, 대규모 corpora에 대한 offline profiling으로 중요도를 계산하고, 85% 가량의 중요하지 않은 layer들은 pruning하여 CPU-NPU synchronization overhead를 줄였다고 한다. 여기에서 layer를 pruning했다는 것은 CPU 연산을 별도로 하지 않고 단순히 NPU에서 연산하도록 했음을 말한다.
...
<!-- 논문에서 처음에는 CPU랬다가, 뒤에는 CPU/GPU라고 해서 살짝 혼동이 있다. -->

![](/assets/img/posts/2025-08-09-llm-npu/수식1.png){: width="450" }

llm.npu는 max-min symmetry quantization을 사용해 구현했고 또한 이를 바탕으로 논문에서 설명하고 있지만, 단순히 outlier만 추출해서 따로 연산하는 것이므로, 임의의 per-tensor quantization과 호환될 수 있다고 한다.

<!-- max-min symmetry quantization이란? -->
<!-- 모델에 적용된 quantization 기법에 llm.npu가 종속적으로 구현되어야 할 것 같다. 논문에서 이야기하는 것처럼 per-tensor quantization이면 유효할 것 같기는 하다. quantization은 inference engine에서의 기법인가, 아니면 모델 구성에서의 기법인가, 아니면 둘 다인가 -->
...
<!-- 여기에서도 CPU/GPU라고 하는데.. 그림엔 또 CPU만 쓰여 있다. shadow 연산은 CPU에서, 나머지 FP 연산은 GPU도 활용하는.. 식으로 동작하는건가? -->
<!-- NPU execution에 따라 CPU execution과 weight retreival이 hidden된다고 했는데.. 이 그림에서는 안 숨겨지는 거 같은데? -->
...
여기에서 $C$는 contribution 값, $g$는 subgraph, $S$는 g가 실행된 직후에 실행될 수 있는 subgraph들의 집합, $T$는 해당 subgraph의 실행 시간이다. g가 CPU/GPU 연산에 대한 subgraph인 경우 직후 연산은 NPU 연산이므로 실행 시간이 길수록 stall이 적은 것으로 판단하고, g가 NPU 연산에 대한 subgraph인 경우 직후 연산은 CPU/GPU 연산이므로 실행 시간이 짧을수록 stall이 적은 것으로 판단한다.

![](/assets/img/posts/2025-08-09-llm-npu/ooo 수식.png){: width="450" }

<!-- 위 수식에서 알 수 있듯이 논문에서는 NPU 연산과 CPU/GPU 연산이 항상 번갈아 나오는 것을 가정하고 있는데, 이 부분이 헷갈린다. -->
<!-- 실제로 NPU 연산 이후 후보 연산에 CPU/GPU 연산만 있는 게 아니라, 그냥 판단을 이렇게 한다는 것 같기도 하다. -->
<!-- 근데 또 그렇다기엔 NPU 이후에 NPU 후보 중 길이 긴 거 고르면 판단이 되는 거 아닌가..? 그냥 후보 연산이 이렇게 구성될 수밖에 없는 이유가 있나? -->

## Implementation & Evaluation

### 개요

llm.npu는 Qualcomm Hexagon NPU에 대해 MLLM(CPU)과 QNN(NPU)을 활용해 구현했다. 또한 device로는 Redmi K70 Pro(Snapdragon 8gen3, 24GB memory)와 Redmi K60 Pro(Snapdragon 8gen2, 16GB memory) 각각에 대해 실험했고, 모두 android os 13으로 실험했다. LLM으로는 Qwen1.5-1.8B, Gemma2B, Phi2-2.7B, LLaMA2-Chat-7B, Mistral7B에 대해 실험했다.
...
실험에서는 TFLite, MMN, llama.cpp, MLC-LLM, PowerInfer-v2로 총 5개의 baseline들과의 비교를 수행했다. 참고로 PowerInfer-v2는 NPU를 활용하여 prefill phase를 가속화하는 engine인데, 오픈소스가 아니어서 해당 논문의 결과를 그대로 가져와 비교했다고 한다. 또한 각 enigne이 실험하려는 LLM들 중 일부만 지원하기도 하므로, 해당 값들은 결과에서 빠져 있다. 참고로 PowerInfer-v2는 prefill phase 가속에 NPU를 사용하기는 하지만, LLM에 대한 최적화를 llm.npu만큼 잘 하지는 못했다고 한다.

<!-- 실제로 동일한 환경에서 실험하지도 못했고, 왜 더 나은지 설명이 되면 좋을 거 같다. -->
...
prefill performance는 speed와 energy consumption으로 측정했고, 2개의 device와 5가지 LLM에 대해 prompt length를 64, 256, 1024로 해서 수행했다. 이때 여러 dataset에 대해서 prefill preformace 향상이 크지 않아서, LongBench dataset으로 실험했다고 한다. 

<!-- 각 데이터셋의 특징이 궁금하다. 특히 longbench는 뭐가 다른 것? -->
...
<!-- 64, 256, 1024같은 딱 떨어질 거 같은 값들로 실험하면 평균적인 경우에 비해 NPU 성능이 좋게 나오는 거 아닌가? 특히 chunk lenght가 256이므로.. padding도 256, 1024에선 안 들어갈 것이고.. -->
...
<!-- 각 engine이 CPU, GPU, NPU 중 뭘 활용하는지 다시 확인해서 명시해두자. -->

### Inference Accuracy

inference accuracy는 5가지 LLM에 대해 5개의 benchmark를 FP16(quantization x), K-Quant(llama.cpp), SmoothQuant(SOTA per-tensor 기법), LLM.Int8(SOTA outlier 처리 기법)과 비교했다.

<!-- 각 기법이 뭔지 알아보면 좋을 것 같다. -->
